...
**In K3**
- when a function word is preceded by a non-nasal sound, which include 300 instances of [p] and 110 instances of [w]
- surface [p] and surface [w] are not equally likely
- In order to model the data of K3, we need a probabilistic grammar which can predict the exact lenition rates
...
- Classical Harmonic Grammar
  - constraint weights and violations are used to calculate a Harmony score for each canidates = acts as a kind of penalty
  - outcome = least-penalized member of GEN for a given input
    - only produces one single winne,r just like OT, it is not a probabilistic theory
