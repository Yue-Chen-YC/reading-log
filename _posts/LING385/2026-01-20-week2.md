...
- systems learned about applying dynamic programing and SGD to solve these problems
  ...
  - reinformecement learning and interaction between sgd
    - we are estmating what the rewards are
    - 2017 systems become really good at this, so good that producing the next word = solving real technological problems like translation
    - this is already a very sucessful techonology = people got excieted about how to make these systems to do conversation
      - give 6 words, and ask them to predict the next word
      - using the word they produce = gpt = generative pretrain transformer, pretrain by learning the reward strcuture

    but they were still very myopic
    - terriable about conversation, when people tried to do this, you ask a question as a prompt, the questions has 19 words and take the 20 words, you put them in again and predict the next words
    - seems like nothing what happens today
    - system uses the prompt to hang around certain areas of mcst and its just stays in this
    - maybe about data? old systems are trained with less data = amount of data is the problem but it's also more than that
    - the response is grammatical = meaning that the words follow each other in the same way an english speaker would put words together, this grammaticality is essential for machine translaiton

  how they solve the problem?
  - human feedback
  - chat gpt has instrcuted gpt, reinformacement learning with human feedback
  - associaiton with ai
  - monte calcure search tree = whole path of words you go through, now ask the system to produce the next word and make it generate the whole trajectory of whole sentence
  - becuase they are chance based systems, they can produce another trajectory
